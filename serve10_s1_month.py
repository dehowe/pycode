...
def s1_month_Combine(path): #整合csv
    if not os.path.exists(path + '月\\'):
        os.mkdir(path + '月\\')
    path_input=path+'天\\'
    path_out=path+'月\\'
    try:
    ...
    month = data_csv.iloc[:, 4]
    year = data_csv.iloc[0, 3]-2020
    month_index=year*12+month[0]+1
    result = []
    j=0
    for i in range(len(data_csv)):
        index=(data_csv.iloc[i, 3]-2020)*12+month[i]
    ...
    if(length!=0):
        ...
        data_correct_df.columns = ['sline','trainid','date','year','month','day','hour','min','week','m1_qy_v','m1_qy_a','m1_qy','m1_zs','m1_fz_v','m1_fz_a','m1_fz','m1_xf_v','m1_xf_a','m1_xf',
                        ...
                        'm3_ebf','m4_ebf','m5_ebf','mc6_ebf','mc1_abf','m2_abf','m3_abf','m4_abf','m5_abf','mc6_abf','motor_tem','accelerate','distance',
                        'passenger','dh_p','dh_c','flag']
        data_correct_df.to_csv(path_out + 'S1010_month.csv', index=0, header=1)
