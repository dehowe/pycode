...
class Data_manager1():
    ...
    def get_path_filename(self,db):
        file_sql = "select * from energyfile_ftp_s1 WHERE flag=0"
...
def Main():
    ...
    #返回ftp对象
    try:
        ftp = ftpconnect("219.242.115.140", 1111, "Administrator", "Qwer1234")
    ...
    if(len(filedata)>0):
        ...
        #清除标记
        delet_sql = "DELETE FROM energyfile_ftp_s1 WHERE flag=0"  # 删除
        db.cursor.execute(delet_sql)
        #导入标记
        sqls_insert_datas = "insert into energyfile_ftp_s1(path,filename,flag) values(%s,%s,%s);"
