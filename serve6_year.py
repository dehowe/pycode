...
class Data_manager4():
    ...
    def get_year_data(self,data):
        if len(data) != 0:
            ...
            year_begin = data_csv.iloc[0, 4]
            result = []
            j = 0
            data_flag=0
            for i in range(len(data_csv)):
                year_index = data_csv.iloc[i, 4]
                if year_begin != year_index:
    ...
    def data_manager3_auto(self,db):
        ...
        print('处理line1_S403中..')
        line1_403_data_day_sql = "select * from sub_edata_day WHERE sline='1'"
        line1_403_data = self.get_mysql_data(line1_403_data_day_sql, db)
        result_line1_403_week = self.get_year_data(line1_403_data)
        self.linedata_year_output_div(result_line1_403_week, db)
        print('处理line6_009中..')
        line6_009_data_day_sql = "select * from sub_edata_day WHERE sline='6'"
        line6_009_data = self.get_mysql_data(line6_009_data_day_sql, db)
        result_line6_009_week = self.get_year_data(line6_009_data)
        self.linedata_year_output_div(result_line6_009_week, db)
        print('处理line7_016中..')
        ...
        result_line7_016_week = self.get_year_data(line7_016_data)
        self.linedata_year_output_div(result_line7_016_week, db)
        print('处理line7_032中..')
        ...
        result_line7_032_week = self.get_year_data(line7_032_data)
        self.linedata_year_output_div(result_line7_032_week, db)
        print('处理line8_066中..')
        ...
        result_line8_066_week = self.get_year_data(line8_066_data)
        self.linedata_year_output_div(result_line8_066_week, db)
        print('处理line8_079中..')
        ...
        result_line8_079_week = self.get_year_data(line8_079_data)
        self.linedata_year_output_div(result_line8_079_week, db)
        print('处理line13_417中..')
        line13_417_data_day_sql = "select * from sub_edata_day WHERE sline='13'"
        line13_417_data = self.get_mysql_data(line13_417_data_day_sql, db)
        result_line13_417_week = self.get_year_data(line13_417_data)
        self.linedata_year_output_div(result_line13_417_week, db)
        print('处理line15_031中..')
        ...
        result_line15_031_week=self.get_year_data(line15_031_data)
        self.linedata_year_output_div(result_line15_031_week, db)
        ...
        result_line17_010_week = self.get_year_data(line17_010_data)
        self.linedata_year_output_div(result_line17_010_week, db)
        ...
        result_line21_019_week = self.get_year_data(line21_019_data)
        self.linedata_year_output_div(result_line21_019_week, db)
        ...
        result_line21_001_week = self.get_year_data(line21_001_data)
        self.linedata_year_output_div(result_line21_001_week, db)
        print('处理line23_437中..')
        ...
        result_line23_437_week = self.get_year_data(line23_437_data)
        self.linedata_year_output_div(result_line23_437_week, db)
        print('年能耗提取结束')
    def lineedata_year_output(self, data_df, database):  # day数据导出到表
        ...
        sqls_insert_datas = "insert into sub_edata_year(sline,trainid,bid,date,year,month,day,hour,min,week,qy1,zs1,qy2,zs2,qy3,zs3,qy4,zs4,qy5,zs5,qy6,zs6,qy7,zs7,qy8,zs8,fz1,fz2,fz3,fz4,qynh,fznh,zsnl,lcsh,yxlc,yxlc_cab,cgl,rs) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
        database.insert_datas(sqls_insert_datas, data_df_list)
    def linedata_year_output_div(self, data_df, database):
        divide = len(data_df)
        begin_index = 0
        end_index = 1
        for i in range(divide):
            self.lineedata_year_output(data_df.iloc[begin_index:end_index, :], database)
            begin_index = begin_index + 1
            end_index = end_index + 1
        self.lineedata_year_output(data_df.iloc[begin_index:, :], database)
    def clear_year_table(self,db):
        sql_linex_week = 'truncate table sub_edata_year'  # 清空
